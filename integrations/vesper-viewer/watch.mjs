...
function stop(){if(closed)return;closed=true;abort.abort();cleanup();if(!check)process.stdout.write('\nTerminal view closed. Vesper and Codex keep running.\n');}
process.once('SIGINT',stop);process.once('SIGTERM',stop);
try {
  if(!process.env.VESPER_DATA_DIR)throw new Error('Set VESPER_DATA_DIR to the running companion data directory.');
  const launch=JSON.parse(await readFile(join(process.env.VESPER_DATA_DIR,'launch.json'),'utf8'));
  const {origin,headers}=connection(launch);
  const readState=async()=>{
    const response=await fetch(origin+'/api/state',{headers,signal:AbortSignal.timeout(5000),redirect:'error'});
    if(!response.ok)throw new Error('Cannot read Vesper state. Open Vesper with its launcher.');
    return response.json();
  };
  const state=await readState();
  if(!state.selected)throw new Error('Select a task in Vesper first.');
  const task=state.selected,expected=process.argv.find(a=>a.startsWith('--thread='))?.slice(9);
  if(expected&&expected!==task.id)throw new Error('Vesper has a different task selected. Select the requested thread first.');
  ...
  if(!check){
    view.line('CODEX × VESPER  /  LIVE TERMINAL VIEW','36');
    ...
    view.line('Read-only: speak or type in Vesper. Closing this window does not stop work.');
    const history=(task.history||[]).slice(-2);
    if(history.length){view.line('\nContext captured when Vesper attached:');for(const m of history){const text=(m.content||[]).map(c=>c.text||'').join('\n');view.text(m.role==='user'?'YOU':'CODEX','history',text+'\n');}}
  ...
  while(!abort.signal.aborted){
    try {
      ...
      if(current.selected?.id!==task.id){view.line('Vesper changed tasks. Reopen this view to follow the new selection.','33');break;}
